...
UPDATE varcharmaptable SET varcharvarintmap['Vitrum edere possum, mihi non nocet.'] = 1010010101020400204143243 WHERE varcharkey= '᚛᚛ᚉᚑᚅᚔᚉᚉᚔᚋ ᚔᚈᚔ ᚍᚂᚐᚅᚑ ᚅᚔᚋᚌᚓᚅᚐ᚜'
    ...
    def test_unicode_invalid_request_error(self):
        ...
        assert 'Keyspace name must not be empty, more than 48 characters long, or contain non-alphanumeric-underscore characters' in err
...
""")
    ...
    def strip_default_ttl(self, describe_statement):
        """
        Remove default_time_to_live option from output of DESCRIBE
        statements. The resulting string may be reused as a CREATE
        MATERIALIZED VIEW statement.
        Useful for tests that recreate materialized views from the
        output of a DESCRIBE statement. DESCRIBE MATERIALIZED VIEW
        statements include default_time_to_live but CREATE
        MATERIALIZED VIEW statements may not include
...
Tracing session:""")
    ...
    def test_materialized_view(self):
        ...
        create_statement = self.strip_default_ttl(self.strip_read_repair_chance(create_statement))
